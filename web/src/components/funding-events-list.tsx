...
export function FundingEventsList(props: FundingEventsListProps) {
  const { q, sub_sector, from, to, page = 1, limit = 10, enabled } = props
  ...
  React.useEffect(() => {
    setCurrentPage(1)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [q, sub_sector, from, to, limit])
  const { events, isLoading, isError, error, count } = useFundingEvents({ q, sub_sector, from, to, page: currentPage, limit }, isEnabled)
  ...
  return (
    <section aria-labelledby="funding-events-title" data-testid="funding-events">
      <div className="flex items-end justify-between mb-2">
        <div>
          <h2 id="funding-events-title" className="text-lg font-medium">Recent Funding Events</h2>
      ...
      {isEnabled && !isLoading && !isError && events.length > 0 && (
        <ul className="divide-y divide-[color:var(--border)] rounded border border-[color:var(--border)] overflow-hidden">
          {events.map((e: FundingEvent, idx: number) => (
            <li key={e.id ?? idx} className="px-4 py-3 text-sm flex items-center gap-4">
